...
function Room(){
    ...
    async function handleCreateRoom(){
        ...
        //Decode jwt token get userId (verification happens on backend)
        //imp: jwt.decode() doesn't verify the signature - that's done on the server
        ...
        try{
            const response = await axios.post(`http://loacalhost:3001/createroom/${slugCreate}`,{
                adminId: userId,
            })
        ...
        <div className="min-h-screen items-center justify-center ml-115 mt-40">
            ...
            <div className="bg-zinc-900 p-8 rounded-xl w-full max-w-md" >
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 bg-zinc-800 text-white rounded-e-lg border border-zinc-700 focus:border-blue-500 focus:outline-none mb-4"
                />
                {error && <p className="text-red-400 text-sm mb-4" >
                 {error}</p>}
